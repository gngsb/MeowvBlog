...
namespace Meowv.Blog.BackgroundJobs.Jobs.HotNews
{
    public class HotNewsJob : IBackgroundJob
    {
        ...
        public async Task ExecuteAsync()
        {
            ...
            {
                new HotNewsJobItem<string> { Result = "https://www.cnblogs.com/legacy", Source = HotNewsEnum.cnblogs },
            ...
            hotnewsUrls.ForEach(item => 
            {
                var task = Task.Run(async () => 
                {
                    ...
                    else if (item.Source == HotNewsEnum.v2ex)
                    {
                        using var client = _httpClient.CreateClient();
                        client.DefaultRequestHeaders.Add("user-agent", "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4238.0 Safari/537.36");
                        var httpResponse = await client.GetAsync(item.Result);
                        obj = await httpResponse.Content.ReadAsStringAsync();
                    }
                    else
                    {
            ...
            var hotNews = new List<Meowv.Blog.HotNews.HotNews>();
            try 
            {
                #region 循环解析数据
                foreach (var list in list_task)
                {
                    ...
                    if (item.Source == HotNewsEnum.cnblogs)
                    {
                        ...
                        nodes.ForEach(x =>
                        {
                            hotNews.Add(new Blog.HotNews.HotNews
                            {
                                Title = x.InnerText,
                    ...
                    if (item.Source == HotNewsEnum.v2ex)
                    {
                        ...
                        htmlDoc.LoadHtml(item.Result.ToString());
                        var nodes = htmlDoc.DocumentNode.SelectNodes("//span[@class='item_title']/a").ToList();
                        nodes.ForEach(x =>
                        {
                            hotNews.Add(new Blog.HotNews.HotNews
                            {
                    ...
                    if (item.Source == HotNewsEnum.juejin)
                    {
                        ...
                        foreach (var node in nodes)
                        {
                            hotNews.Add(new Blog.HotNews.HotNews
                            {
                                Title = node["item_info"]["article_info"] != null ? node["item_info"]["article_info"]["title"].ToString() : node["item_info"]["title"].ToString(),
                                Url = node["item_info"]["article_info"] != null ? node["item_info"]["article_info"]["link_url"].ToString() : node["item_info"]["url"].ToString(),
                                SourceId = sourceId,
                                CreateTime = DateTime.Now
                            });
                        }
